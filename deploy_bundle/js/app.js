...
// Reduced-motion mode: visitors with prefers-reduced-motion set get each
// animated SVG's .still twin (its finished frame, generated by
// build_stills.py) instead of the SMIL animation. Flipping the OS setting
// re-renders the verse in place.
...
function renderVerse(index) {
  ...
  const imgPath =
    visual && prefersReducedMotion() && visual.stillUrl
      ? visual.stillUrl
      : visual
        ? visual.url
        : null;
