...
let lastRenderedIndex = null; // for re-rendering when display prefs change
let stageTextToken = 0; // invalidates in-flight text-visual fetches

// Still-graphics mode: each animated SVG's .still twin (its finished
// frame, generated by build_stills.py) instead of the SMIL animation. The
// "Still graphics" checkbox is the switch; it defaults to the visitor's
// OS prefers-reduced-motion setting, and once they touch it their explicit
// choice wins and persists. Flipping the OS setting only moves the
// checkbox while they haven't chosen.
const STILL_KEY = "avbible.web.stillGraphics";
const REDUCED_MOTION = window.matchMedia
  ? window.matchMedia("(prefers-reduced-motion: reduce)")
  : null;

function prefersReducedMotion() {
  return Boolean(REDUCED_MOTION && REDUCED_MOTION.matches);
}

function rerenderCurrentVerse() {
  if (!passage || lastRenderedIndex === null) return;
  if (lastRenderedIndex < passage.verses.length) {
    renderVerse(lastRenderedIndex);
  }
}

function initMotionToggle() {
  const stored = localStorage.getItem(STILL_KEY);
  els.chkMotion.checked =
    stored !== null ? stored === "1" : prefersReducedMotion();
  els.chkMotion.addEventListener("change", () => {
    localStorage.setItem(STILL_KEY, els.chkMotion.checked ? "1" : "0");
    rerenderCurrentVerse();
  });
  if (REDUCED_MOTION && REDUCED_MOTION.addEventListener) {
    REDUCED_MOTION.addEventListener("change", () => {
      if (localStorage.getItem(STILL_KEY) !== null) return; // user chose
      els.chkMotion.checked = REDUCED_MOTION.matches;
      rerenderCurrentVerse();
    });
  }
}
...
function renderVerse(index) {
  ...
  const visual = stage.findVisual(ctx);
  const imgPath =
    visual && els.chkMotion.checked && visual.stillUrl
      ? visual.stillUrl
      : visual
        ? visual.url
        : null;
